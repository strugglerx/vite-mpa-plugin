...
**`load` 链顺序（子页/目录型入口）：** 读盘 → `transformHtml`（`phase: 'load'`）→ 若 `styleInline !== false` 则 `replaceIndexStyle` → `replaceInject`。  
**仅「键为 `*.html` 的直连型」入口：** 读盘 → `transformHtml` → `replaceInject`（无 `replaceIndexStyle`）。  
