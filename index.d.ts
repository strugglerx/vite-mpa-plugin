...
export interface MpaPluginConfig {
	...
	debug?: boolean
	/**
	 * 为 `false` 时关闭：开发服务器（`vite` / `pnpm run dev`）启动时在控制台打印 `build.rollupOptions.input` 各**键**对应的页面 URL（与 `config.base` 拼接）及源 `*.html` 路径。
	 * @default true（仅 `serve` 时打印；`vite build` 不打印）
	 */
	logInputMap?: boolean
	/**
